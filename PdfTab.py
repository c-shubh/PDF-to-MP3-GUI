from tkinter import (ttk, W, E, filedialog, messagebox)
...
class PdfTab:
    ...
    def selectPdf(self):
        ...
        if self.selectedPdfPath != '':
            ...
            self.toggleState("!disabled", self.w.saveMp3Button,
                             self.w.saveTextButton, self.w.playMp3Button)
            self.w.selectedFileDisplayVar.set(textwrap.fill(
                os.path.basename(self.selectedPdfPath), width=19))
    ...
    def saveMp3(self):
        ...
        if(self.filePathToSaveMp3 != ''):
            ...
            self.c.saveMp3FromPdf(self.w.maleFemaleRadioButtonVar.get(),
                                  self.w.normalSlowFastRadioButtonVar.get(), self.selectedPdfPath, self.filePathToSaveMp3)
            messagebox.showinfo(
    ...
    def saveText(self):
        ...
        if(self.filePathToSaveText != ''):
            messagebox.showinfo(
                "Information", f"Please wait while {os.path.basename(self.filePathToSaveText)} is being saved")
            self.c.saveTextFromPdf(self.selectedPdfPath,
                                   self.filePathToSaveText)
            messagebox.showinfo(
