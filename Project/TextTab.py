from tkinter import (ttk, W, E, filedialog, Text,
                     StringVar, Scrollbar, END, messagebox)
...
class TextTab:
    def __init__(self, tabs):
        ...
        self.w.saveMp3Button.grid(
            row=10, column=0, sticky='w', padx=5, pady=(15, 0))

        self.w.saveTextButton.grid(
            row=11, column=0, sticky='w', padx=5, pady=(10, 0))

        self.w.playMp3Button.grid(
            row=12, column=0, sticky='w', padx=5, pady=(10, 15))
        ...
        self.checkBoxForSelectFileOrWriteText = ttk.Checkbutton(
            self.w.rightFrame, text="From File", takefocus=False, variable=self.checkBoxForSelectFileOrWriteTextVar, command=self.fromFileOrText)

        self.checkBoxForSelectFileOrWriteText.grid(
        ...
        self.clearTextButton = ttk.Button(
            self.w.rightFrame, text="Clear Text", style="clearButton.TButton", command=self.clearTextConfirmAlertBox)
        self.clearTextButton.grid(
    ...
    def selectText(self):
        ...
        if self.selectedTextFilePath != '':
            ...
            self.w.saveMp3Button["state"] = "!disabled"
            self.w.playMp3Button["state"] = "!disabled"
            self.w.selectedFileDisplayVar.set(textwrap.fill(
                os.path.basename(self.selectedTextFilePath), width=19))
    ...
    def saveMp3(self):
        if(self.checkBoxForSelectFileOrWriteTextVar.get() == '1'):
            self.filePathToSaveMp3 = filedialog.asksaveasfilename(
                filetypes=[('MP3 files', '*.mp3')], defaultextension='.mp3', initialfile='myfile')
            self.infoToUserToWait()
            self.c.saveMp3FromTextFile(self.w.maleFemaleRadioButtonVar.get(),
                                       self.w.normalSlowFastRadioButtonVar.get(), self.selectedTextFilePath, self.filePathToSaveMp3)
            self.infoToUserForFileSaved()
        else:
            self.filePathToSaveMp3 = filedialog.asksaveasfilename(
                filetypes=[('MP3 files', '*.mp3')], defaultextension='.mp3', initialfile='myfile')
            self.infoToUserToWait()
            self.c.saveMp3FromInputText(self.w.maleFemaleRadioButtonVar.get(),
                                        self.w.normalSlowFastRadioButtonVar.get(), self.inputTextArea.get("1.0", END), self.filePathToSaveMp3)
            self.infoToUserForFileSaved()

    def infoToUserToWait(self):
        messagebox.showinfo(
            "Information", f"Please wait while {os.path.basename(self.filePathToSaveMp3)} is being saved")
    ...
    def saveText(self):
        self.filePathToSaveText = filedialog.asksaveasfilename(
            filetypes=[('TEXT files', '*.txt')], defaultextension='.txt', initialfile='myfile')
        self.c.saveTextFromInput(self.inputTextArea.get(
            "1.0", END), self.filePathToSaveText)
